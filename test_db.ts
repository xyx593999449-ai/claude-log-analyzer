...
async function main() {
  ...
  const dataAll = await repo.getOverview({ 
    page: 1, pageSize: 20, search: '', verifyStatus: '', qcStatus: '', alertTags: [], manualOnly: false, anomalyOnly: false, batches: [] 
  });
  ...
  const dataFiltered = await repo.getOverview({ 
    page: 1, pageSize: 20, search: '', verifyStatus: '', qcStatus: '', alertTags: [], manualOnly: false, anomalyOnly: false, batches: [testUuid] 
  });
