...
- 已完成独立路由与顶部导航入口接入，可从主导航直接进入 HITL 迭代模块。
- 页面已补齐页首总览区，展示迭代批次标识、时间范围、样本量与当前状态，便于快速掌握本轮范围与进度。
- 页面已提供与现有 dashboard 视觉语言一致的 HITL 流程骨架区，覆盖`人工反馈结果池 -> LLM 分析 -> 双 Skill 建议 -> 候选版本 -> 联合回归验证 -> 发布结论`六阶段。
- 数据模型已明确区分 `人工反馈结果池`、`迭代批次` 与 `任务执行批次`，后续页面实现将基于该契约逐步填充。
