...
export function ToolAnalysisView({ stats }: { stats: LogStats }) {
  ...
  return (
    <div className="ide-scrollbar max-h-[56vh] overflow-auto rounded-xl border border-slate-200 bg-slate-50 p-3">
        ...
        {rows.map((row) => (
          <div
            ...
            className={`flex items-center justify-between rounded-lg border px-3 py-2 shadow-sm ${
              row.error > 0 ? "border-rose-200 bg-rose-50" : "border-slate-200 bg-white"
            }`}
          >
            <div className="font-medium text-slate-700">{row.name}</div>
            <div className={`rounded-full px-2 py-0.5 ${row.error > 0 ? "bg-rose-100 text-rose-700" : "bg-slate-100 text-slate-600"}`}>
