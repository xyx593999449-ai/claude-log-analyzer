...
- **HITL 迭代页路由与导航入口**：新增 `/hitl-iterations` 独立路由、顶部导航按钮与页面骨架，占位呈现 HITL 闭环阶段并保持与现有 dashboard 一致的视觉风格。
- **HITL 迭代页总览与流程骨架区**：页首新增迭代批次标识、时间范围、样本量、状态卡片，并补齐六阶段流程骨架（人工反馈结果池、LLM 分析、双 Skill 建议、候选版本、联合回归验证、发布结论）。
